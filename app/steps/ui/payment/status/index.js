...
class PaymentStatus extends Step {
    ...
    * runnerOptions(ctx, session) {
        ...
        if (serviceAuthResult.name === 'Error') {
            ...
            options.url = `${this.steps.PaymentBreakdown.constructor.getUrl()}?status=serviceAuthError`;
            return options;
        }
        ...
        if (ctx.paymentDue) {
            ...
            if (getPaymentResponse.name === 'Error' || getPaymentResponse.status === 'Initiated') {
                ...
                const options = {};
                options.redirect = true;
                options.url = `${this.steps.PaymentBreakdown.constructor.getUrl()}?status=${getPaymentResponse.status}`;
                return options;
            }
